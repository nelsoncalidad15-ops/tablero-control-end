...
const QuestionCard = ({ label, question, records }: { label: string; question: QuestionKey; records: ScoringRecord[] }) => {
  ...
  return <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm"><p className="min-h-10 text-[10px] font-black uppercase leading-4 tracking-[.14em] text-slate-500">{label}</p><div className="relative mx-auto h-36 max-w-[190px]"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={chart.length ? chart : [{ name: 'Sin datos', value: 1, color: '#e2e8f0' }]} dataKey="value" innerRadius={38} outerRadius={56} paddingAngle={3} stroke="none">{(chart.length ? chart : [{ name: 'Sin datos', value: 1, color: '#e2e8f0' }]).map((item) => <Cell key={item.name} fill={item.color} />)}</Pie><Tooltip formatter={(value: number, name: string) => [`${value} clientes`, name]} /></PieChart></ResponsiveContainer><div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center"><span className="text-2xl font-black tracking-tighter">{percent(pct(yes, yes + no + unknown))}</span><span className="text-[8px] font-black uppercase tracking-[.18em] text-teal-600">Sí</span></div></div><div className="grid grid-cols-3 gap-2 text-center text-[10px] font-bold"><span className="rounded-xl bg-teal-50 px-2 py-2 text-teal-700">Sí {yes}</span><span className="rounded-xl bg-rose-50 px-2 py-2 text-rose-700">No {no}</span><span className="rounded-xl bg-slate-100 px-2 py-2 text-slate-500">S/D {unknown}</span></div></div>;
};
...
const ScoringDashboard: React.FC<ScoringDashboardProps> = ({ onBack }) => {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900"><div className="mx-auto w-full max-w-[1680px] px-4 py-5 md:px-6 md:py-7">
    ...
    {loading ? <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{Array.from({ length: 8 }).map((_, index) => <div key={index} className="h-32 animate-pulse rounded-3xl bg-slate-200" />)}</div> : error ? <div className="mt-5 rounded-3xl border border-rose-200 bg-rose-50 p-6 text-rose-800"><div className="flex items-start gap-3"><Icons.AlertCircle className="mt-.5 h-5 w-5" /><div><p className="font-black">No se pudieron cargar las respuestas de scoring.</p><p className="mt-1 text-sm">{error}</p><p className="mt-3 text-xs">En Render verificá <code className="rounded bg-white px-1.5 py-1 font-bold">LINK_SCORING</code> y el permiso de lectura de la cuenta de servicio.</p></div></div></div> : <>
      <section className="mt-7"><div className="mb-3"><p className="text-[10px] font-black uppercase tracking-[.2em] text-teal-600">Comprensión del proceso</p><h2 className="mt-1 text-xl font-black tracking-tight">Primera respuesta de cada cliente</h2><p className="mt-1 text-sm text-slate-500">Este análisis muestra dónde reforzar la explicación comercial desde el primer contacto.</p></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{QUESTIONS.map((question) => <QuestionCard key={question.key} label={question.label} question={question.key} records={analysisRecords} />)}</div></section><section className="mt-4 rounded-[1.75rem] border border-rose-100 bg-rose-50 p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.2em] text-rose-700">Focos a reforzar</p><p className="mt-1 text-sm text-slate-600">Aspectos con más clientes que respondieron No en su primera entrevista.</p></div><div className="grid gap-2 sm:grid-cols-3">{focusPoints.map((item) => <div key={item.key} className="flex items-center justify-between gap-3 rounded-xl bg-white px-3 py-2.5 shadow-sm"><span className="text-xs font-bold text-slate-700">{item.label}</span><span className="shrink-0 text-xs font-black text-rose-700">{item.no} No · {percent(item.rate)}</span></div>)}</div></div></section>      <section className="mt-7"><div className="mb-4"><p className="text-[10px] font-black uppercase tracking-[.2em] text-amber-600">Montos por modelo</p><h2 className="mt-1 text-xl font-black tracking-tight">Valores informados en la primera entrevista</h2><p className="mt-1 text-sm text-slate-500">La mediana evita que un valor aislado distorsione la lectura. Los nombres se normalizan para comparar el mismo vehículo.</p></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{modelAmounts.map((item) => <div key={item.model} className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm"><p className="text-[10px] font-black uppercase tracking-[.18em] text-slate-500">{item.model}</p><p className="mt-3 text-xs font-semibold text-slate-500">Cuota 2 informada</p><p className="mt-1 text-lg font-black tracking-tight text-slate-900">{money(item.estimated)}</p><p className="mt-3 text-xs font-semibold text-slate-500">Primera cuota</p><p className="mt-1 text-lg font-black tracking-tight text-slate-900">{money(item.firstPayment)}</p></div>)}</div></section>
