...
const ScoringDashboard: React.FC<ScoringDashboardProps> = ({ onBack }) => {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    <motion.header initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="sticky top-0 z-40 flex min-h-[58px] items-center gap-3 border-b border-slate-200/80 bg-white/95 px-3 py-2 shadow-sm backdrop-blur-xl md:px-5">
      <button onClick={onBack} className="rounded-xl border border-slate-200 bg-white p-2 text-slate-400 shadow-sm transition hover:border-blue-100 hover:bg-blue-50 hover:text-blue-600" title="Volver a Ventas"><Icons.ArrowLeft className="h-4 w-4" /></button>
      <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-slate-950 text-white shadow-lg shadow-slate-900/20"><Icons.Target className="h-4 w-4" /></div>
      <div className="min-w-0"><h1 className="truncate text-sm font-bold leading-none tracking-[-.035em] text-[#001e50] md:text-lg">Scoring de clientes</h1><p className="mt-1.5 text-[7px] font-bold uppercase tracking-[.16em] text-slate-400">Planes de ahorro</p></div>
    </motion.header>
    <div className="mx-auto w-full max-w-[1680px] px-3 py-4 md:px-5 md:py-5">
