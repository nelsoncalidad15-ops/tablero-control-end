...
const ScoringDashboard: React.FC<ScoringDashboardProps> = ({ onBack }) => {
  ...
  }), [records, branch, fromDate, toDate, model, advisor, result, contact]);

  const filtered = useMemo(() => selectedVersion(scopedRecords, mode), [scopedRecords, mode]);
  const latestClients = useMemo(() => selectedVersion(scopedRecords, 'latest'), [scopedRecords]);
  const firstClients = useMemo(() => selectedVersion(scopedRecords, 'first'), [scopedRecords]);
  ...
  const metrics = useMemo(() => {
    ...
    return {
      ...
      advisors: new Set(filtered.map((record) => record.advisor).filter(Boolean)).size,
      models: new Set(filtered.map((record) => record.model).filter(Boolean)).size,
    };
  }, [filtered, latestClients, firstClients]);
  ...
  const reset = () => { setBranch('all'); setMode('all'); setFromDate(''); setToDate(''); setModel(''); setAdvisor(''); setResult(''); setContact(''); };
  return <div className="min-h-screen bg-slate-50 text-slate-900"><div className="mx-auto w-full max-w-[1680px] px-4 py-5 md:px-6 md:py-7">
    ...
    {loading ? <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{Array.from({ length: 8 }).map((_, index) => <div key={index} className="h-32 animate-pulse rounded-3xl bg-slate-200" />)}</div> : error ? <div className="mt-5 rounded-3xl border border-rose-200 bg-rose-50 p-6 text-rose-800"><div className="flex items-start gap-3"><Icons.AlertCircle className="mt-.5 h-5 w-5" /><div><p className="font-black">No se pudieron cargar las respuestas de scoring.</p><p className="mt-1 text-sm">{error}</p><p className="mt-3 text-xs">En Render verificá <code className="rounded bg-white px-1.5 py-1 font-bold">LINK_SCORING</code> y el permiso de lectura de la cuenta de servicio.</p></div></div></div> : <>
      ...
      <section className="mt-5 grid gap-4 lg:grid-cols-[1.15fr_.85fr]"><div className="h-full rounded-3xl border border-blue-100 bg-blue-50 p-5"><p className="text-[10px] font-black uppercase tracking-[.18em] text-blue-700">Primera respuesta por cliente</p><div className="mt-4 grid grid-cols-3 gap-3"><div className="rounded-2xl bg-white/80 p-3"><p className="text-[9px] font-black uppercase tracking-[.14em] text-emerald-700">Aceptados</p><p className="mt-1 text-2xl font-black">{metrics.firstAccepted}</p></div><div className="rounded-2xl bg-white/80 p-3"><p className="text-[9px] font-black uppercase tracking-[.14em] text-amber-700">Pendientes</p><p className="mt-1 text-2xl font-black">{metrics.firstPending}</p></div><div className="rounded-2xl bg-white/80 p-3"><p className="text-[9px] font-black uppercase tracking-[.14em] text-rose-700">Rechazados</p><p className="mt-1 text-2xl font-black">{metrics.firstRejected}</p></div></div></div><div className="flex h-full flex-col justify-between rounded-3xl border border-teal-100 bg-teal-50 p-5"><p className="text-[10px] font-black uppercase tracking-[.18em] text-teal-700">Recuperados</p><p className="mt-2 text-4xl font-black tracking-tighter">{metrics.recovered}</p><p className="mt-2 text-sm leading-5 text-slate-600">Clientes rechazados en su primera respuesta y aceptados en la última.</p></div></section>      <section className="mt-7"><div className="mb-3"><p className="text-[10px] font-black uppercase tracking-[.2em] text-teal-600">Comprensión del proceso</p><h2 className="mt-1 text-xl font-black tracking-tight">Respuestas Sí / No</h2></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{QUESTIONS.map((question) => <QuestionCard key={question.key} label={question.label} question={question.key} records={filtered} />)}</div></section>      <section className="mt-7 grid gap-5 xl:grid-cols-[1.15fr_.85fr]"><div className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm"><p className="text-[10px] font-black uppercase tracking-[.2em] text-indigo-600">Origen de la propuesta</p><h2 className="mt-1 text-xl font-black tracking-tight">Cómo conocieron la propuesta</h2><div className="mt-4 h-[320px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={origins} margin={{ top: 10, right: 12, left: -20, bottom: 50 }}><XAxis dataKey="origin" angle={-28} textAnchor="end" interval={0} tick={{ fontSize: 10 }} /><YAxis allowDecimals={false} tick={{ fontSize: 10 }} /><Tooltip /><Legend /><Bar dataKey="JUJUY" name="Jujuy" fill="#2563eb" radius={[6, 6, 0, 0]} /><Bar dataKey="SALTA" name="Salta" fill="#f97316" radius={[6, 6, 0, 0]} /></BarChart></ResponsiveContainer></div></div><div className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm"><p className="text-[10px] font-black uppercase tracking-[.2em] text-amber-600">Otro plan reciente</p><h2 className="mt-1 text-xl font-black tracking-tight">Detalles más repetidos</h2><div className="mt-5 space-y-2">{otherPlans.length ? otherPlans.map((item) => <div key={item.detail} className="flex items-center justify-between gap-4 rounded-2xl bg-slate-50 px-3 py-3"><p className="line-clamp-2 text-sm font-semibold text-slate-700">{item.detail}</p><span className="shrink-0 rounded-full bg-amber-100 px-2 py-1 text-xs font-black text-amber-700">{item.count}</span></div>) : <p className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">No hay detalle de otro plan con los filtros actuales.</p>}</div></div></section>
